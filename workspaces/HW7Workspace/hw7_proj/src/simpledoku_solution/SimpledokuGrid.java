...
public class SimpledokuGrid 
{
	...
	public boolean isLegal()
	{
		...
		for (int row = 0; row < values.length; row ++)
		  {
		       ...
		       for (int column = 0; column < values[0].length; column++)
		       {
		           ...
		           if(containsNonZeroRepeats(ints2) == true)
		   		{
		   			return false;
		   		}
		       }
		...
		for (int row = 0; row < values.length; row ++)
		  {
			ints3.add(values[row][row]);
	           if(containsNonZeroRepeats(ints3) == true)
	   		{
	   			return false;
	   		}	
		  }
		...
		for (int row = values.length-1; row >= 0 ; row --)
		  {
			...
			ints4.add(values[row][column]);
	           if(containsNonZeroRepeats(ints4) == true)
	   		{
	   			return false;
	   		}	
			column++;
	...
	public static void main(String[] args)
	{
		...
		sop(gridOne.values.length);
		sop(gridOne.values[2][1]);
		sop(gridOne.isLegal());
		
		for(SimpledokuGrid s:gridOne.generateNextGrids() )
		{
			sop(s);
		}
	}
